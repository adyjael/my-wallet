...
$levantamentos = Connection::connect()->prepare("SELECT * FROM take_money WHERE user_id = ?");
$levantamentos->execute(array($userId));
$levantamentos = $levantamentos->fetchAll(PDO::FETCH_ASSOC);
        ...
        <section role="chart" class="exchange-rates">
            ...
            <?php if (count($levantamentos) > 0) : ?>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php foreach ($levantamentos as $levantamento) :  ?>

                            <tr>
                                <td scope="row"><?= $levantamento["id"] ?></td>
                                <td>$<?= $levantamento["amount"] ?></td>
                                <td><?= $levantamento["motivo"] ?></td>
                                <td><?= date("d/m/Y",strtotime($levantamento["date"])) ?></td>
                                <td><?= date("H:i",strtotime($levantamento["date"])) ?></td>
                            </tr>
                        <?php endforeach ?>
                    </tbody>
                </table>
            <?php else :  ?>
                ...
                <h4 class="text-center">Você ainda não fez nenhum Levantamento</h4>
            <?php endif  ?>
...
<script>
    ...
    $.ajax({
        ...
        if(dados.length < 5){
            $(".title").html("Para ver o grafico precisa de 5 Levantamentos")
        }else {
            $(".title").html("Grafico de Saidas")
